...
d3.csv("https://raw.githubusercontent.com/ripleycleghorn/thesis/main/project-code/all_emissions.csv").then(data =>{
    ...
    var historicArray = data.filter(d => d.Entity == 'historic')
    var filteredArray = data.filter(d => d.numericYear < 1970 && d.numericYear > 1860)
    ...
    console.log(d3.extent(historicArray, d => d.year))
    // Set the ranges
    var xScale = d3.scaleTime()
        .domain([startDate, d3.max(historicArray, d => d.year)])
        .range([0, width]);
    var yScale = d3.scaleLinear()
        .domain(d3.extent(historicArray, d => d.emissions))
        .range([height, 0]);
